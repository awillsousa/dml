# modified from https://github.com/lisa-lab/DeepLearningTutorials/blob/master/code/SdA.py
...
def test_SdA(finetune_lr=0.1, pretraining_epochs=pretraining_epochs_g,
             pretrain_lr=0.001, training_epochs=training_epochs_g,
...
def predict_custom_image_sda(params, testImgFilename='own_0.png', activation=activation_g):

    test_img = fli.processImg('../data/custom/', testImgFilename)
    test_img_value = filter(str.isdigit, testImgFilename)

    hidden_input = test_img
    for j in range(len(params)//2 -1):
        hidden_input = activation(T.dot(hidden_input, params[2*j]) + params[2*j + 1])

    final_output = T.dot(hidden_input, params[3*j]) + params[3*j+1]

    p_y_given_x = T.nnet.softmax(final_output)
    y_pred = T.argmax(p_y_given_x, axis=1)
    testfunc = theano.function([], [y_pred])
