...
nr_examples= 200
modelfilename = '../data/nnfromscratch_eq_2' + str(nr_examples) + '.pkl'
...
def calculate_loss(model, X = inputs):
    ...
    data_loss = np.sqrt(np.sum((z3-y)**2))
...
def predict(model, x=inputs, res = y):
    ...
    #exp_scores = np.exp(z2)
    #probs = exp_scores / np.sum(exp_scores, axis=1, keepdims=True)
    print("mean error " + str(np.mean(np.abs(z3-res))))
    return z3
